...
pub struct PanOrbitState {
    ...
    pub pitch: f32,
    pub yaw: f32,
    pub needs_transform_refresh: bool,
}
...
///  );
/// ```
pub fn _toggle_auto_orbit(mut pan_orbit_camera: Query<&mut PanOrbitSettings>) {
...
pub fn update_pan_orbit_camera(
    ...
    for (settings, mut state, mut transform) in &mut q_camera {
        ...
        if settings.auto_orbit || state.needs_transform_refresh || any_change || state.is_added() {
            state.needs_transform_refresh = false;
